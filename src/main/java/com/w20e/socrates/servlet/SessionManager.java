...
public class SessionManager {
    ...
    public final HttpSession getSession(final HttpServletRequest req) {
        ...
        if ((!hasSession(req)) && req.getParameter("id") != null) {
    ...
    private String getInstanceFileFromLongSession(final String surveyId,
        ...
        try {
            return cookie.getValue().split(SessionManager.LONG_SESSION_SEPARATOR)[0];
        } catch (Exception e) {
            LOGGER.severe("Couldn't extract instance file from cookie with value " + cookie.getValue());
        }
    ...
    public void invalidateSession(final HttpServletRequest req) {
        ...
        try {
            ...
            LOGGER.log(Level.WARNING, "Couldn't invalidate session: "
                    + e.getMessage());
        }
    }
    ...
    public Instance salvageInstance(final String surveyId,
        ...
        final String instancefile = getInstanceFileFromLongSession(surveyId, req);
        final String id = getSessionIdFromLongSession(surveyId, req);

        if (instancefile == null || id == null) {
            LOGGER
                    .severe("No instancefile nor original session id found in long session");
            return null;
        }
            ...
            try {
                ...
                if (new File(file).exists()) {
                    try {
                        ...
                        LOGGER
                        .severe("Exception in resolving file. This is unusual...");
                    }
                } else {
    ...
    private String getSessionIdFromLongSession(final String surveyId,
        ...
        try {
            return cookie.getValue().split(SessionManager.LONG_SESSION_SEPARATOR)[1];
        } catch (Exception e) {
            LOGGER.severe("Couldn't extract session id from cookie with value " + cookie.getValue());
        }
